...
let leaves=JSON.parse(localStorage.getItem("payLeaves")||"null")||[];
...
function save(){
 ...
 localStorage.setItem("payExpenses",JSON.stringify(expenses));
  localStorage.setItem("payLeaves",JSON.stringify(leaves));
  localStorage.setItem("payLeaves",JSON.stringify(leaves));
 ...
 if(window.nosaDB && cloudLoaded && !cloudWriting){
   cloudWriting=true;
   const state={employees,attendance,loans,adjustments,overtime,expenses,leaves,branchCodes,settings,updatedAt:Date.now()};
...
function applyCloudState(v){
 ...
 expenses=Array.isArray(v.expenses)?v.expenses:[];
  leaves=Array.isArray(v.leaves)?v.leaves:[];
  leaves=Array.isArray(v.leaves)?v.leaves:[];
 ...
 localStorage.setItem("paySettings",JSON.stringify(settings));
 prepForms();
 renderDashboard(); renderEmployees(); renderAttendanceAllEmployees(); renderLeaves(); renderLoans(); renderAdjustments(); renderOvertime(); renderPayroll(); renderExpenses(); renderBranchPortal(); if(sessionEmployee) renderEmployeePortalWithLeaves();
}
function initCloud(){
...
function formatLateDuration(total){
 let n=Math.max(0,Number(total)||0),h=Math.floor(n/60),m=n%60;
 return h?(h+" ساعة"+(m?" و "+m+" دقيقة":"")):(m+" دقيقة");
}
function lateInfo(e,inTime,periodEnd=today(),cycle=e.cycle){
 let actual=minutes(inTime),scheduled=minutes(e.start||settings.start);
 if(actual==null||scheduled==null||actual<=scheduled)return {minutes:0,deduction:0,durationLabel:"0 دقيقة"};
 let lm=actual-scheduled;
 let base=salaryForPeriod(e,periodRange(periodEnd,cycle));
 let rates=periodPayRates(e,base,cycle);
 return {minutes:lm,hourlyRate:rates.hourlyRate,perMinute:rates.perMinute,deduction:lm*rates.perMinute,durationLabel:formatLateDuration(lm)};
}
...
function periodPayRates(e,base,cycle){
 let shift=diffMin(e.start||settings.start,e.end||settings.end)||1;
 let days=cycle==="شهري"?30:cycle==="15 يوم"?15:7;
 let hourlyRate=(Number(base)||0)/(days*(shift/60));
 return {shift,days,hourlyRate,perMinute:hourlyRate/60};
}
function calcPayroll(e,end=e.cycle==="شهري"?today():today(),cycle=e.cycle){
 ...
 let late=0,lateDed=0,early=0,earlyDed=0;
 let rates=periodPayRates(e,base,cycle);
 let shift=rates.shift,days=rates.days,hourlyRate=rates.hourlyRate,perMinute=rates.perMinute;

 at.forEach(a=>{
   // الإجازة المعتمدة تلغي خصم الحضور والانصراف لهذا اليوم بالكامل،
   // مع الاحتفاظ بسجل الدخول والانصراف نفسه وعدم حذفه.
   if(approvedLeaveForDate(e.id,a.date)) return;

   let scheduledIn=minutes(e.start||settings.start);
   let scheduledOut=minutes(e.end||settings.end);
   let actualIn=minutes(a.in);
   let actualOut=minutes(a.out);

   if(actualIn!=null && scheduledIn!=null && actualIn>scheduledIn){
     let lm=actualIn-scheduledIn;
     late+=lm;
     lateDed+=lm*perMinute;
   }
   if(actualOut!=null && scheduledOut!=null){
     let em=scheduledOut-actualOut;
     if(em>0){
       early+=em;
       earlyDed+=em*perMinute;
     }
   }
 });
 ...
 let net=Math.max(0,base+ot-lateDed-earlyDed-deduction-penalties-loan);
 return {e,r,base,late,lateDed,early,earlyDed,deduction,penalties,loan,ot,net,hourlyRate,perMinute};
}
...
// ===== إضافات الإجازات وسجل الحضور التاريخي — لا تلغي أي وظيفة موجودة =====
function isApprovedLeave(lv){
  return !!lv && (lv.status==="approved" || lv.status==="معتمدة") && lv.status!=="cancelled";
}
function approvedLeaveForDate(employeeId,d){
  return leaves.find(lv=>lv.employeeId===employeeId&&isApprovedLeave(lv)&&d>=lv.startDate&&d<=lv.endDate);
}
function leaveCoversDate(lv,d){
  return isApprovedLeave(lv) && d>=lv.startDate && d<=lv.endDate;
}
function employeeOnLeave(e,d){
  return !!approvedLeaveForDate(e.id,d);
}
function employeeLeaveRanges(e){
  return leaves.filter(lv=>lv.employeeId===e.id&&lv.status!=="cancelled")
    .sort((a,b)=>b.startDate.localeCompare(a.startDate));
}
function renderLeaves(){
  let d=$("#leaveDateFilter")?.value||"all";
  let eid=$("#leaveEmployeeFilter")?.value||"all";
  let st=$("#leaveStatusFilter")?.value||"all";
  let list=leaves.filter(lv=>
    (d==="all" || leaveCoversDate(lv,d)) &&
    (eid==="all" || lv.employeeId===eid) &&
    (st==="all" || lv.status==="approved")
  ).sort((a,b)=>b.startDate.localeCompare(a.startDate));
  $("#leavesTable").innerHTML=list.length?list.map(lv=>{
    let e=emp(lv.employeeId);
    return `<tr>
      <td>${fmtDate(lv.startDate)}</td><td>${fmtDate(lv.endDate)}</td>
      <td><b>${esc(e?.name||"—")}</b></td><td>${esc(e?.branch||"—")}</td>
      <td>${esc(lv.type||"إجازة")}</td><td>${esc(lv.reason||"—")}</td>
      <td><span class="badge green">معتمدة</span></td>
      <td><div class="actions"><button class="mini-btn" data-edit-leave="${lv.id}">تعديل</button>
      <button class="mini-btn danger" data-del-leave="${lv.id}">حذف</button></div></td>
    </tr>`;
  }).join(""):`<tr><td colspan="8" class="muted">لا توجد إجازات مطابقة.</td></tr>`;
}
function populateLeaveEmployeeSelect(){
  let html=employees.map(e=>`<option value="${e.id}">${esc(e.name)} — ${esc(e.code)}</option>`).join("");
  if($("#lvEmployee")) $("#lvEmployee").innerHTML=html;
  if($("#leaveEmployeeFilter")) $("#leaveEmployeeFilter").innerHTML=`<option value="all">كل الموظفات</option>${html}`;
}
function openLeave(id=null){
  ...
  $("#lvStart").value=today();
  $("#lvEnd").value=today();
  $("#leaveModalTitle").textContent="إضافة إجازة";
  if(id){
    let lv=leaves.find(x=>x.id===id);
    if(!lv)return;
    $("#lvId").value=lv.id;
    $("#lvEmployee").value=lv.employeeId;
    $("#lvType").value=lv.type||"إجازة عادية";
    $("#lvStart").value=lv.startDate;
    $("#lvEnd").value=lv.endDate;
    $("#lvReason").value=lv.reason||"";
    $("#leaveModalTitle").textContent="تعديل الإجازة";
  }
  openModal("leaveModal");
}
function leaveDeductionForPayroll(e,r){
  // الإجازة المعتمدة لا تحمل أي خصم. الدالة موجودة لتوضيح القاعدة
  // ومنع أي كود مستقبلي من اعتبار الإجازة خصماً تلقائياً.
  return 0;
}
function renderAttendanceAllEmployees(){
  let d=$("#attDate").value||today(),b=$("#attBranch").value,s=$("#attStatus").value;
  let active=employees.filter(e=>e.active&&(b==="all"||e.branch===b));
  let rows=active.map(e=>{
    let a=attendance.find(x=>x.employeeId===e.id&&x.date===d);
    let onLeave=employeeOnLeave(e,d);
    let lm=a?.in?Math.max(0,minutes(a.in)-minutes(e.start||settings.start)):0;
    let om=a?.out?Math.max(0,minutes(a.out)-minutes(e.end||settings.end)):0;
    let status=onLeave?"إجازة":!a?.in?"لم يسجل":(lm?"متأخر":"حاضر");
    if(s==="present" && !(a?.in && !lm && !onLeave)) return "";
    if(s==="late" && !(a?.in && lm && !onLeave)) return "";
    if(s==="absent" && (a?.in||onLeave)) return "";
    return `<tr>
      <td>${fmtDate(d)}</td><td><b>${esc(e.name)}</b></td><td>${esc(e.branch)}</td>
      <td>${a?.in?formatTimeArabic(a.in):"—"}</td><td>${a?.out?formatTimeArabic(a.out):"—"}</td>
      <td>${lm?formatLateDuration(lm):"0 دقيقة"}</td><td>${lm?money(lateInfo(e,a.in).deduction):money(0)}</td>
      <td>${a?.out?`${Math.floor(om/60)}س ${om%60}د`:"—"}</td>
      <td><span class="badge ${onLeave?"purple":!a?.in?"red":lm?"orange":"green"}">${status}</span>
      ${onLeave?`<small class="leave-inline"> لا خصم</small>`:""}</td>
      <td>${a?`<button class="mini-btn" data-edit-att="${a.id}">تعديل</button>`:
        `<button class="mini-btn" data-add-att-for="${e.id}">تسجيل</button>`}</td>
    </tr>`;
  }).filter(Boolean).join("");
  $("#attendanceTable").innerHTML=rows||`<tr><td colspan="11" class="muted">لا توجد سجلات مطابقة.</td></tr>`;
}
function renderEmployeePortalWithLeaves(){
  let current=sessionEmployee?emp(sessionEmployee.id):null;if(current)sessionEmployee=current;
  let e=sessionEmployee, end=today(),x=calcPayroll(e,end,e.cycle);
  let todayA=attendance.filter(a=>a.employeeId===e.id).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,12);
  let leaveRows=employeeLeaveRanges(e).slice(0,8).map(lv=>
    `<div class="emp-row"><b>${fmtDate(lv.startDate)} → ${fmtDate(lv.endDate)}</b>
     <span>${esc(lv.type||"إجازة")} — ${esc(lv.reason||"بدون ملاحظة")}</span>
     <strong class="leave-note">إجازة معتمدة — بدون خصم</strong></div>`).join("");
  let todayRecord=attendance.find(a=>a.employeeId===e.id&&a.date===end);
  let late=todayRecord?.in?lateInfo(e,todayRecord.in):{minutes:0,deduction:0};
  $("#empName").textContent=e.name;$("#empWelcome").textContent=e.name;
  $("#empBranch").textContent=`${e.branch} — مواعيد العمل ${formatTimeArabic(e.start)} إلى ${formatTimeArabic(e.end)}`;
  $("#empCode").textContent="";$("#empPayCycle").textContent=e.cycle;$("#empSalary").textContent=money(e.salary);
  $("#empLoans").textContent=money(x.loan);$("#empNet").textContent=money(x.net);
  $("#empAttendanceList").innerHTML=(todayA.length?todayA.map(a=>{
    let leaveForDay=approvedLeaveForDate(e.id,a.date); let li=(!leaveForDay&&a.in)?lateInfo(e,a.in):{minutes:0,deduction:0,durationLabel:"0 دقيقة"};
    let admin=a.recordedByAdmin?`<small class="admin-attendance-note">تم اعتماد/تعديل الوقت من الإدارة</small>`:"";
    let movements=[];
    if(a.in) movements.push(`<span class="movement-entry"><b>الدخول</b>: ${formatTimeArabic(a.in)}</span>`);
    if(a.out) movements.push(`<span class="movement-entry"><b>الانصراف</b>: ${formatTimeArabic(a.out)}</span>`);
    if(!movements.length) movements.push(`<span class="movement-entry">لا يوجد تسجيل دخول أو انصراف</span>`);
    return `<div class="emp-row"><b>${fmtDate(a.date)}</b><div class="movement-list">${movements.join("")}</div>${admin}${li.minutes?`<strong class="late-note">تأخير ${li.durationLabel||formatLateDuration(li.minutes)} — خصم ${money(li.deduction)}</strong>`:""}</div>`;
  }).join(""):"<div class=\"muted\">لا توجد سجلات.</div>")+
  (leaveRows?`<div class="leave-section"><h4>إجازاتي المعتمدة</h4>${leaveRows}</div>`:"");
  let periodAdj=adjustments.filter(a=>a.employeeId===e.id&&inRange(a.date,x.r));
  let reasonRows=periodAdj.map(a=>`<div class="reason-item"><span>${esc(a.type)} — ${esc(a.reason||"بدون سبب")}</span><span>${money(a.amount)}</span></div>`).join("");
  $("#empPayrollDetails").innerHTML=`${late.minutes?`<div class="employee-late-alert"><b>⚠️ تأخير اليوم: ${late.durationLabel||formatLateDuration(late.minutes)}</b><span>الخصم المحسوب: ${money(late.deduction)}</span></div>`:""}<div class="emp-row"><span>الأساسي</span><b>${money(x.base)}</b></div><div class="emp-row"><span>إجمالي التأخير</span><b>${formatLateDuration(x.late)} — ${money(x.lateDed)}</b></div><div class="emp-row"><span>الانصراف المبكر</span><b>${formatLateDuration(x.early)} — ${money(x.earlyDed)}</b></div><div class="emp-row"><span>الخصومات والجزاءات</span><b>${money(x.deduction+x.penalties)}</b></div>${reasonRows?`<div class="reason-box"><b>تفاصيل أسباب الخصومات والجزاءات</b>${reasonRows}</div>`:""}<div class="emp-row"><span>السلف</span><b>${money(x.loan)}</b></div><div class="emp-row"><span>الأوفر تايم</span><b>${money(x.ot)}</b></div><div class="emp-row"><span>الإجازات المعتمدة</span><b>${approvedLeaveForDate(e.id,end)?"اليوم إجازة معتمدة — لا يوجد خصم حضور أو انصراف":"لا يوجد خصم"}</b></div><div class="emp-row"><span>الصافي</span><b>${money(x.net)}</b></div>`;
}
...
function renderAttendance(){
 ...
 list=list.filter(a=>{
   let e=emp(a.employeeId);
   if(s==="all")return true;
   if(s==="absent")return !a.in;
   return a.in&&minutes(a.in)>minutes(e?.start||settings.start);
 });
 list.sort((a,b)=>b.date.localeCompare(a.date));
 $("#attendanceTable").innerHTML=list.length?list.map(a=>{
   let e=emp(a.employeeId);
   let onLeave=approvedLeaveForDate(a.employeeId,a.date);
   let lm=(!onLeave&&a.in)?Math.max(0,minutes(a.in)-minutes(e?.start||settings.start)):0;
   let earlyMin=(!onLeave&&a.out&&e?.end&&minutes(a.out)<minutes(e.end))?minutes(e.end)-minutes(a.out):0;
   let lateAmount=lm?lateInfo(e,a.in,a.date,e.cycle).deduction:0;
   let earlyAmount=earlyMin?earlyMin*(periodPayRates(e,salaryForPeriod(e,periodRange(a.date,e.cycle)),e.cycle).perMinute):0;
   return `<tr>
    <td>${fmtDate(a.date)}</td><td>${esc(nameOf(a.employeeId))}</td><td>${e?.branch||""}</td>
    <td>${a.in?formatTimeArabic(a.in):"—"}</td><td>${a.out?formatTimeArabic(a.out):"—"}</td>
    <td>${onLeave?"0 دقيقة":(lm?formatLateDuration(lm):"0 دقيقة")}</td>
    <td>${onLeave?money(0):(lm?money(lateAmount):money(0))}</td>
    <td>${onLeave?"—":(earlyMin?formatLateDuration(earlyMin):"—")}</td>
    <td>${onLeave?money(0):(earlyMin?money(earlyAmount):money(0))}</td>
    <td><span class="badge ${onLeave?"green":!a.in?"red":lm?"orange":"green"}">${onLeave?"إجازة معتمدة":!a.in?"غائب":lm?"متأخر":"حاضر"}</span></td>
    <td><button class="mini-btn" data-edit-att="${a.id}">تعديل</button> <button class="mini-btn danger" data-del-att="${a.id}">حذف</button></td>
   </tr>`;
 }).join(""):`<tr><td colspan="11" class="muted">لا توجد سجلات مطابقة.</td></tr>`;
}
...
function renderPayroll(){
 let selected=$("#payrollCycle").value,end=$("#payrollEnd").value||today();
 let list=employees
   .filter(e=>e.active&&(selected==="all"||e.cycle===selected))
   .map(e=>calcPayroll(e,end,e.cycle));
 lastPayroll=list;
 $("#payrollTable").innerHTML=list.map(x=>`<tr>
   <td><b>${esc(x.e.name)}</b><br><span class="muted">${x.e.branch} — ${x.e.cycle}</span></td>
   <td>${money(x.base)}</td>
   <td>${money(x.lateDed)}<br><span class="muted">${formatLateDuration(x.late)}</span></td>
   <td>${money(x.earlyDed)}<br><span class="muted">${formatLateDuration(x.early)}</span></td>
   <td>${money(x.deduction)}</td><td>${money(x.penalties)}</td><td>${money(x.loan)}</td><td>${money(x.ot)}</td>
   <td><b>${money(x.net)}</b></td><td><button class="mini-btn" data-pay-report="${x.e.id}">كشف</button></td>
 </tr>`).join("")||`<tr><td colspan="10" class="muted">لا يوجد موظفون على هذا الاختيار.</td></tr>`;
 let sums=list.reduce((a,x)=>{a.base+=x.base;a.d+=x.lateDed+x.earlyDed+x.deduction+x.penalties+x.loan;a.ot+=x.ot;a.net+=x.net;return a},{base:0,d:0,ot:0,net:0});
...
function renderReport(){
 let id=$("#reportEmployee").value,end=$("#reportEnd").value||today(),e=emp(id);
 if(!e){$("#reportPreview").innerHTML="";return}
 // كشف المرتب يجب أن يستخدم نظام قبض الموظفة نفسها، وليس اختياراً مختلفاً،
 // حتى يكون مبلغ خصم التأخير والانصراف المبكر مطابقاً 100% لصفحة الموظفة.
 let cycle=e.cycle;
 $("#reportCycle").value=cycle;
...
function downloadJPG(x){
 ...
 const rows=[["المرتب الأساسي",money(x.base)],["الأوفر تايم",money(x.ot)],["خصم التأخير",money(x.lateDed)],["خصم الانصراف المبكر",money(x.earlyDed)],["الخصومات",money(x.deduction)],["الجزاءات",money(x.penalties)],["السلف",money(x.loan)],["صافي المستحق",money(x.net)]];
...
function showPage(page){
 ...
 const titles={dashboard:"لوحة المؤشرات",employees:"الموظفين",attendance:"الحضور والانصراف",leaves:"الإجازات",payroll:"المرتبات",loans:"السلف",adjustments:"الخصومات والجزاءات",overtime:"الأوفر تايم",expenses:"مصروفات الفروع",reports:"الكشوفات والتقارير",settings:"الإعدادات"};
 $("#pageTitle").textContent=titles[page];$("#sidebar").classList.remove("open");
 if(page==="dashboard")renderDashboard();if(page==="expenses")renderExpenses();if(page==="employees")renderEmployees();if(page==="attendance")renderAttendanceAllEmployees();if(page==="leaves"){populateLeaveEmployeeSelect();renderLeaves();}if(page==="loans")renderLoans();if(page==="adjustments")renderAdjustments();if(page==="overtime")renderOvertime();if(page==="payroll")renderPayroll();if(page==="reports")renderReport();
}
function openModal(id){$("#"+id).classList.add("show")}
function closeModal(id){$("#"+id).classList.remove("show")}
function prepForms(){
 populateEmployeeSelect("#aEmployee");populateEmployeeSelect("#lEmployee");populateEmployeeSelect("#xEmployee");populateEmployeeSelect("#oEmployee");populateLeaveEmployeeSelect();
...
$("#loginForm").onsubmit=e=>{
 ...
 if(loginMode==="admin"){
   ...
   sessionEmployee=e;sessionStorage.setItem("paySession",JSON.stringify({role:"employee",employeeId:e.id}));$("#loginScreen").classList.add("hidden");$("#employeePortal").classList.remove("hidden");renderEmployeePortalWithLeaves();
 }
};
...
$("#attendanceForm").onsubmit=e=>{
 e.preventDefault();
 let id=$("#aId").value||uid("a");
 let old=attendance.find(x=>x.id===id);
 let obj={
   ...(old||{}),
   id,
   employeeId:$("#aEmployee").value,
   date:$("#aDate").value,
   in:normalizeTime($("#aIn").value)||old?.in||"",
   out:normalizeTime($("#aOut").value)||old?.out||"",
   note:$("#aNote").value||old?.note||"",
   recordedByAdmin:true,
   adminEditedAt:Date.now()
 };
 let i=attendance.findIndex(x=>x.id===id);
 if(i>=0)attendance[i]=obj;
 else{
   let same=attendance.findIndex(x=>x.employeeId===obj.employeeId&&x.date===obj.date);
   if(same>=0)attendance[same]=obj;else attendance.push(obj);
 }
 save();
 closeModal("attendanceModal");
 if(typeof renderAttendanceAllEmployees==="function")renderAttendanceAllEmployees();else renderAttendance();
 renderDashboard();
 if(sessionEmployee && sessionEmployee.id===obj.employeeId){
   if(typeof renderEmployeePortalWithLeaves==="function")renderEmployeePortalWithLeaves();
   else if(typeof renderEmployeePortal==="function")renderEmployeePortal();
 }
 toast("تم حفظ وقت الحضور والانصراف وإظهاره للموظفة");
};
$("#attendanceTable").onclick=e=>{let add=e.target.closest("[data-add-att-for]");if(add){prepForms();$("#attendanceForm").reset();$("#aId").value="";$("#aEmployee").value=add.dataset.addAttFor;$("#aDate").value=$("#attDate").value||today();openModal("attendanceModal");return}let a=e.target.closest("[data-edit-att]"),d=e.target.closest("[data-del-att]");if(a){let x=attendance.find(q=>q.id===a.dataset.editAtt);populateEmployeeSelect("#aEmployee");$("#aId").value=x.id;$("#aEmployee").value=x.employeeId;$("#aDate").value=x.date;$("#aIn").value=x.in?normalizeTime(x.in):"";$("#aOut").value=x.out?normalizeTime(x.out):"";$("#aNote").value=x.note||"";openModal("attendanceModal")}if(d&&confirm("حذف سجل الحضور؟")){attendance=attendance.filter(x=>x.id!==d.dataset.delAtt);save();renderAttendanceAllEmployees();renderDashboard()}};
...
// ===== إدارة الإجازات من الإدارة فقط =====
$("#addLeave").onclick=()=>openLeave();
$("#leaveDateFilter").value=today();
$("#leaveDateFilter").onchange=renderLeaves;
$("#leaveEmployeeFilter").onchange=renderLeaves;
$("#leaveStatusFilter").onchange=renderLeaves;
$("#leavesTable").onclick=e=>{
  let ed=e.target.closest("[data-edit-leave]"),dl=e.target.closest("[data-del-leave]");
  if(ed) openLeave(ed.dataset.editLeave);
  if(dl && confirm("حذف سجل الإجازة؟")){
    leaves=leaves.filter(x=>x.id!==dl.dataset.delLeave);
    save(); renderLeaves(); renderEmployeePortalWithLeaves(); toast("تم حذف سجل الإجازة");
  }
};
$("#leaveForm").onsubmit=e=>{
  e.preventDefault();
  let id=$("#lvId").value||uid("lv");
  let start=$("#lvStart").value,end=$("#lvEnd").value;
  if(!start||!end||end<start){toast("تأكد من تاريخ بداية ونهاية الإجازة");return}
  let obj={id,employeeId:$("#lvEmployee").value,type:$("#lvType").value,startDate:start,endDate:end,reason:$("#lvReason").value.trim(),status:"approved",createdAt:Date.now()};
  let i=leaves.findIndex(x=>x.id===id);
  if(i>=0) leaves[i]=obj; else leaves.push(obj);
  save(); closeModal("leaveModal"); renderLeaves(); renderAttendanceAllEmployees();
  if(sessionEmployee) renderEmployeePortalWithLeaves();
  toast(i>=0?"تم تعديل الإجازة":"تم اعتماد الإجازة — بدون خصم");
};
...
function renderEmployeePortal(){
 ...
 let todayRecord=attendance.find(a=>a.employeeId===e.id&&a.date===end), todayLeave=approvedLeaveForDate(e.id,end), late=(!todayLeave&&todayRecord?.in)?lateInfo(e,todayRecord.in,end,e.cycle):{minutes:0,deduction:0,durationLabel:"0 دقيقة"};
 ...
 $("#empAttendanceList").innerHTML=todayA.length?todayA.map(a=>{let leaveForDay=approvedLeaveForDate(e.id,a.date); let li=(!leaveForDay&&a.in)?lateInfo(e,a.in):{minutes:0,deduction:0,durationLabel:"0 دقيقة"};return `<div class="emp-row"><b>${fmtDate(a.date)}</b><span>دخول: ${a.in?formatTimeArabic(a.in):"—"} | خروج: ${a.out?formatTimeArabic(a.out):"—"}</span>${a.recordedByAdmin?`<small class="admin-attendance-note">تم اعتماد/تعديل الوقت من الإدارة</small>`:""}${li.minutes?`<strong class="late-note">تأخير ${li.durationLabel||formatLateDuration(li.minutes)} — خصم ${money(li.deduction)}</strong>`:""}</div>`}).join(""):`<div class="muted">لا توجد سجلات.</div>`;
 ...
 $("#empPayrollDetails").innerHTML=`${late.minutes?`<div class="employee-late-alert"><b>⚠️ تأخير اليوم: ${late.durationLabel||formatLateDuration(late.minutes)}</b><span>الخصم المحسوب: ${money(late.deduction)}</span></div>`:""}<div class="emp-row"><span>الأساسي</span><b>${money(x.base)}</b></div><div class="emp-row"><span>إجمالي التأخير</span><b>${formatLateDuration(x.late)} — ${money(x.lateDed)}</b></div><div class="emp-row"><span>الانصراف المبكر</span><b>${formatLateDuration(x.early)} — ${money(x.earlyDed)}</b></div><div class="emp-row"><span>الخصومات والجزاءات</span><b>${money(x.deduction+x.penalties)}</b></div>${reasonRows?`<div class="reason-box"><b>تفاصيل أسباب الخصومات والجزاءات</b>${reasonRows}</div>`:""}<div class="emp-row"><span>السلف</span><b>${money(x.loan)}</b></div><div class="emp-row"><span>الأوفر تايم</span><b>${money(x.ot)}</b></div><div class="emp-row"><span>الصافي</span><b>${money(x.net)}</b></div>`;
}
...
function employeePunch(type){
 ...
 if(type==="in"){
   ...
   a.in=now; save(); renderEmployeePortalWithLeaves();
   ...
   a.out=now; save(); renderEmployeePortalWithLeaves();
...
setInterval(clock,1000);clock();prepForms();populateLeaveEmployeeSelect();renderLeaves();$("#branchExpenseForm").onsubmit=e=>{e.preventDefault();if(!sessionBranch)return;let amount=Number($("#beAmount").value);if(!(amount>0)){toast("أدخل مبلغ المصروف");return}expenses.push({id:uid("ex"),branch:sessionBranch,date:today(),type:$("#beType").value,amount,reason:$("#beReason").value.trim(),createdAt:Date.now()});save();$("#branchExpenseForm").reset();renderBranchPortal();toast("تم تسجيل المصروف وسماعه في لوحة الإدارة")};
$("#expenseDate").value=today();$("#expenseDate").onchange=renderExpenses;$("#expenseBranchFilter").onchange=renderExpenses;
$("#expensesTable").onclick=e=>{let d=e.target.closest("[data-del-exp]");if(d&&confirm("حذف هذا المصروف؟")){expenses=expenses.filter(x=>x.id!==d.dataset.delExp);save();renderExpenses();renderDashboard();toast("تم حذف المصروف")}};
$("#clearDayExpenses").onclick=()=>{let d=$("#expenseDate").value||today(), b=$("#expenseBranchFilter").value||"all";let count=expenses.filter(x=>x.date===d&&(b==="all"||x.branch===b)).length;if(!count){toast("لا توجد مصروفات للتصفير");return}if(confirm(`سيتم حذف ${count} مصروف من ${b==="all"?"كل الفروع":b} بتاريخ ${fmtDate(d)}. هل أنت متأكد؟`)){expenses=expenses.filter(x=>!(x.date===d&&(b==="all"||x.branch===b)));save();renderExpenses();renderDashboard();toast("تم تصفير المصروفات المحددة")}};
$("#saveBranchCodes").onclick=()=>{let h=$("#branchCodeHadayek").value.trim(),d=$("#branchCodeDawagen").value.trim();if(!h||!d){toast("أدخل كود الفرعين");return}if(h===d){toast("يجب أن يكون كود كل فرع مختلفاً");return}branchCodes={الحدايق:h,الدواجن:d};save();toast("تم حفظ أكواد دخول الفروع")};
...
function restoreSession(){
  ...
  if(savedSession.role==="admin"){
    ...
    if(e && e.active){
      ...
      renderEmployeePortalWithLeaves();
    }else sessionStorage.removeItem("paySession");
  }
}
initCloud();
